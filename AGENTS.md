...
- 로컬 실행: 최초 1회 `cp .env.example .env` 후 값(비밀번호 등)을 설정 → `docker compose up -d` (MySQL + Redis) → `./gradlew bootRun` (local 프로필, 루트 `.env` 로드)
- `./gradlew test`는 Testcontainers가 DB를 직접 띄우므로 `.env`가 필요 없다

## 아키텍처

- 패키지: `com.flab.coongyapay.{account, auth, bank, common, config, kyc, user, wallet}` — 충전·원장 기능 도입과 함께 `{charge, idempotency, reconciliation, transaction}`이 추가된다 (도입 진행 중)
- 레이어: controller → service → repository → mapper(MyBatis XML)
- 도메인 객체는 불변으로 설계한다: private 생성자 + 정적 팩토리 메서드(`create`, `from`), setter 없음
- 지갑 잔액은 원장(ledger) 기반으로 **전환 중**이다. 현재는 `wallet.balance`(+`version`)가 잔액을 보관하며, 목표 구조는 `transaction` 도메인의 거래 헤더 + `TransactionEntry`(CREDIT/DEBIT) append 기록과 DB UNIQUE 제약(이중 기입 방지, 지갑별 순번 무결성)이다
- 오래 걸리는 작업(외부 은행 연동 등)은 워커 + 상태머신 패턴으로 비동기 처리하며, 워커는 lease 선점과 펜싱 토큰으로 중복 실행을 막는다 (원장과 함께 도입 진행 중)
...
아래 규칙은 원장(`transaction`/`TransactionEntry`) 구조를 포함하는 변경에 적용한다.
...
- 멱등 쓰기 경로(멱등키 선점, 원장 이중 기입 방지, 보상 거래 중복 생성 방지 등 UNIQUE가 멱등 장치로 설계된 곳)에서 `DuplicateKeyException`을 "이미 처리됨/경합 패배"로 간주하고 정상 진행하는 패턴은 의도된 안전 경로이므로 지적하지 않는다.
- 반면 비즈니스 제약 UNIQUE(이메일·계좌 중복 등) 위반을 삼키고 정상 진행하는 코드는 결함이다. 이런 위반은 `BusinessException`(예: `DUPLICATE_ACCOUNT`)으로 변환해 호출자에게 알려야 한다.

### 실패 분류

- 외부 호출을 확정 실패(FAILED)로 종결하는 것은 API 계약상 부작용이 없음이 보장된 명시적 거절 응답에 한한다.
- 타임아웃·5xx뿐 아니라 408처럼 처리 결과를 확정할 수 없는 4xx도 결과 불명(UNKNOWN)으로 분류한다. HTTP 상태 코드 클래스만으로 확정/불명을 나누는 코드는 지적한다.
...
- 매퍼 테스트는 `@MybatisTest` 슬라이스 테스트를 사용한다 (자동 롤백으로 공유 Testcontainer 격리). 여러 빈을 엮는 통합 테스트는 `@SpringBootTest` + `@Transactional`을 사용한다.
- 두 경우 모두 롤백 격리 장치(`@MybatisTest`의 자동 롤백, `@Transactional`)를 제거하는 변경은 공유 Testcontainer를 오염시키므로 지적한다.
- 단언은 AssertJ를 사용한다.
